...
<div class="app-shell">
    ...
    <div class="sidebar-overlay" id="sidebar-overlay" onclick="window.closeAppSidebar && window.closeAppSidebar()"></div>

    <aside class="sidebar" id="app-sidebar">
        <button type="button" class="sidebar__close" onclick="window.closeAppSidebar && window.closeAppSidebar()" aria-label="Yopish">&times;</button>
        ...
        <div class="sidebar__brand">
            ...
            <span class="sidebar__brand-text">
                ...
                <small>
                    @auth
                        @if (auth()->user()->role === 'super_admin') Bosh Admin
                        @elseif (auth()->user()->role === 'admin') Admin
                        @elseif (auth()->user()->role === 'chevar') Chevar
                        @elseif (auth()->user()->role === 'ega') {{ App\Models\Company::active()?->nomi ?? '-' }} Ega
                        @else Mijoz
                        @endif
        ...
        <nav class="sidebar__nav">
            <a href="{{ route('admin.dashboard') }}" class="sidebar__link {{ request()->routeIs('admin.dashboard') ? 'is-active' : '' }}" onclick="window.closeAppSidebar && window.closeAppSidebar()">
                <span class="icon">◧</span> <span>Statistika</span>
            </a>
            <a href="{{ route('admin.products.index') }}" class="sidebar__link {{ request()->routeIs('admin.products.*') ? 'is-active' : '' }}" onclick="window.closeAppSidebar && window.closeAppSidebar()">
                <span class="icon">▤</span> <span>Mahsulotlar</span>
            </a>
            <a href="{{ route('admin.xodimlar.index') }}" class="sidebar__link {{ request()->routeIs('admin.xodimlar.*', 'admin.chevar-*') ? 'is-active' : '' }}" onclick="window.closeAppSidebar && window.closeAppSidebar()">
                <span class="icon">◎</span> <span>Xodimlar</span>
            </a>
            @auth
                @if (auth()->user()->role === 'super_admin')
                    <a href="{{ route('admin.companies.index') }}" class="sidebar__link {{ request()->routeIs('admin.companies.*') ? 'is-active' : '' }}" onclick="window.closeAppSidebar && window.closeAppSidebar()">
            ...
            <a href="#" class="sidebar__link"><span class="icon">✎</span> <span>Profil</span></a>
        </nav>
        ...
        <div class="sidebar__foot">
            @auth
                ...
                @if (auth()->user()->role === 'super_admin' && \App\Models\Company::active())
                    <a href="{{ route('admin.companies.index') }}" class="sidebar__link sidebar__switch-company" onclick="window.closeAppSidebar && window.closeAppSidebar()">
    ...
    <div class="main">
        <header class="topbar">
            <button type="button" class="topbar__menu" id="sidebar-toggle"
                    onclick="window.toggleAppSidebar && window.toggleAppSidebar(); return false;"
                    aria-label="Menyu">
